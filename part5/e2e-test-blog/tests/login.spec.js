...
describe('Blog app', () => {
  ...
  describe('When logged in', () => {
    ...
    test('blogs are arranged in the order according to the likes count', async ({ page }) => {
      // create 3 blogs
      const initBlogs = Array(3).fill().map(() => randomBlog())
      ...
      await ldetails.getByRole('button', { name: 'like' }).waitFor({ state: 'visible' })
      for (let i = 0; i < 5; i++) {
        let currentLikes = parseInt(await (await ldetails.getByText('likes').innerHTML()).match(/\d+/)[0])
      ...
      // get all blogs likes
      const llikesAfterSort = await lblogs.locator('xpath=..').getByText('likes').all()
      let likesAfterSort = []
